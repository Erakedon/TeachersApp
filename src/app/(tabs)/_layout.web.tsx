...
function WebTabButton({ children, isFocused, icon, ...props }: WebTabButtonProps) {
  return (
    <Pressable {...props} style={({ pressed }) => pressed && styles.pressed}>
      <ThemedView bg={isFocused ? 'primaryContainer' : 'surfaceContainerLow'} style={styles.tabButton}>
        <Icon
          name={icon}
          size={18}
  ...
  return (
    <View style={styles.tabBarOuter}>
      <ThemedView bg="surfaceContainerLowest" style={styles.tabBarInner}>
        <ThemedText variant="headlineSmall" color="primary" style={styles.brandText}>
          Teacher's App
        </ThemedText>
        {props.children}
      </ThemedView>
